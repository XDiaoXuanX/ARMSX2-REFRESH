...
// [ARMSX2] Note: vu1Thread and CpuMicroVU0/CpuMicroVU1 are now
...
struct RecStubsInit {
    RecStubsInit() {
        ...
        if (diag_enabled) {
             fprintf(stderr, "@@CFG@@ ARMSX2_REC_DIAG=1\n");
        }
        ...
        // [ARMSX2] Diagnostic Hook for recCpu.Execute
        if (diag_enabled) {
            recCpu.Execute = []() {
        ...
        // [ARMSX2] New Config Log
        fprintf(stderr, "@@CFG@@ ARMSX2_REAL_REC=%d ARMSX2_USE_REC_STUBS=%d\n",
            #if defined(ARMSX2_REAL_REC)
            1, 0
            #else
            0, 1
...
// [ARMSX2] Note: vuJITFreeze() is now provided by x86/microVU.cpp
