...
static bool IsFix9JitNopEnabled()
{
	...
	static int s_enabled = -1;
	if (s_enabled < 0)
		s_enabled = ARMSX2_GetRuntimeEnvBool("ARMSX2_FIX9_JIT_NOP", false) ? 1 : 0;
	return (s_enabled == 1);
}
...
static bool IsBeq023d8NoConstEnabled()
{
	...
	static int s_enabled = -1;
	if (s_enabled < 0)
		s_enabled = ARMSX2_GetRuntimeEnvBool("ARMSX2_BEQ023D8_NO_CONST", false) ? 1 : 0;
	return (s_enabled == 1);
}
...
static bool IsEeLoopBranchProbeEnabled()
{
	...
	if (s_enabled < 0)
	{
		const char* env = ARMSX2_GetRuntimeEnv("ARMSX2_EE_LOOP_BRANCH_PROBE");
		s_enabled = (env && env[0] == '1') ? 1 : 0;
		Console.WriteLn("@@CFG@@ ARMSX2_EE_LOOP_BRANCH_PROBE=%d", s_enabled);
	}
	return (s_enabled == 1);
}
...
static bool IsEeLoopNoConstBranchEnabled()
{
	...
	if (s_enabled < 0)
	{
		const char* env = ARMSX2_GetRuntimeEnv("ARMSX2_EE_LOOP_NO_CONST_BRANCH");
		s_enabled = (env && env[0] == '1') ? 1 : 0;
		Console.WriteLn("@@CFG@@ ARMSX2_EE_LOOP_NO_CONST_BRANCH=%d", s_enabled);
	}
	return (s_enabled == 1);
}
...
#ifndef ARMSX2_ENABLE_BRANCH_RUNTIME_PROBES
#define ARMSX2_ENABLE_BRANCH_RUNTIME_PROBES 0
#endif


#if ARMSX2_ENABLE_BRANCH_RUNTIME_PROBES
...
#endif // ARMSX2_ENABLE_BRANCH_RUNTIME_PROBES
...
extern "C" u32 ReadGuestMem32ForLfCmp(u32 addr)
{
	...
	{
		const char* env = ARMSX2_GetRuntimeEnv("ARMSX2_PROBE_LF_MEMREAD");
		s_probe_lf_memread = (env && env[0] == '1') ? 1 : 0;
		Console.WriteLn("@@CFG@@ ARMSX2_PROBE_LF_MEMREAD=%d", s_probe_lf_memread);
	}
...
#ifndef ARMSX2_ENABLE_LOADFILE_CMP_PACK
#define ARMSX2_ENABLE_LOADFILE_CMP_PACK 0  // default OFF: avoid probe-side host register clobber
#endif

#if ARMSX2_ENABLE_LOADFILE_CMP_PACK
struct LfCmpEntry {
...
#ifndef ARMSX2_ENABLE_LF_CMP_USED_PROBE
#define ARMSX2_ENABLE_LF_CMP_USED_PROBE 0
#endif
...
#ifndef ARMSX2_ENABLE_LF_PROBE_PACK_V3
#define ARMSX2_ENABLE_LF_PROBE_PACK_V3 0  // DISABLED for baseline
#endif

#if ARMSX2_ENABLE_LF_PROBE_PACK_V3
...
#ifndef ARMSX2_DISABLE_LF_DELAY_SWAP
#define ARMSX2_DISABLE_LF_DELAY_SWAP 0
#endif
...
void recBEQ()
{
	...
	if (DarwinMisc::ARMSX2_FORCE_JIT_VERIFY && (pc - 4) == 0xBFC023D8)
	{
...
static void recBNE_process(int process)
{
...

	{
		...
		{
			const char* env = ARMSX2_GetRuntimeEnv("ARMSX2_DISABLE_LF_DELAY_SWAP");
			s_disable_lf_delay_swap = (ARMSX2_IsSafeOnlyEnabled() || !IsDiagFlagsEnabled()) ? 0 : ((env && env[0] == '1') ? 1 : 0);
			Console.WriteLn("@@CFG@@ ARMSX2_DISABLE_LF_DELAY_SWAP=%d", s_disable_lf_delay_swap);
		}
...
void recBNE()
{
	...
	const bool allow_lf_branch_compile_log = (!ARMSX2_IsSafeOnlyEnabled() && IsDiagFlagsEnabled());
	...
	// [FIX] Removed ARMSX2_FORCE_JIT_VERIFY override that forced all BNE to process=0.
