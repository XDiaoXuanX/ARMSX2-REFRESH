...
namespace armsx2_fastmem_dbg {
    ...
    static void ensure_init() {
        if (s_init) return;
        s_init = 1;
        s_d8   = std::getenv("ARMSX2_FASTMEM_DISABLE_8")   ? 1 : 0;
        s_d16  = std::getenv("ARMSX2_FASTMEM_DISABLE_16")  ? 1 : 0;
        s_d32  = std::getenv("ARMSX2_FASTMEM_DISABLE_32")  ? 1 : 0;
        s_d64  = std::getenv("ARMSX2_FASTMEM_DISABLE_64")  ? 1 : 0;
        s_d128 = std::getenv("ARMSX2_FASTMEM_DISABLE_128") ? 1 : 0;
        s_d_w  = std::getenv("ARMSX2_FASTMEM_DISABLE_WRITE") ? 1 : 0;
        s_d_r  = std::getenv("ARMSX2_FASTMEM_DISABLE_READ")  ? 1 : 0;
        // [Phase A.2] bit-size + read/write 同時指定の細粒 disable
        s_dw8   = std::getenv("ARMSX2_FASTMEM_DISABLE_WRITE_8")   ? 1 : 0;
        s_dw16  = std::getenv("ARMSX2_FASTMEM_DISABLE_WRITE_16")  ? 1 : 0;
        s_dw32  = std::getenv("ARMSX2_FASTMEM_DISABLE_WRITE_32")  ? 1 : 0;
        s_dw64  = std::getenv("ARMSX2_FASTMEM_DISABLE_WRITE_64")  ? 1 : 0;
        s_dw128 = std::getenv("ARMSX2_FASTMEM_DISABLE_WRITE_128") ? 1 : 0;
        s_dr8   = std::getenv("ARMSX2_FASTMEM_DISABLE_READ_8")    ? 1 : 0;
        s_dr16  = std::getenv("ARMSX2_FASTMEM_DISABLE_READ_16")   ? 1 : 0;
        s_dr32  = std::getenv("ARMSX2_FASTMEM_DISABLE_READ_32")   ? 1 : 0;
        s_dr64  = std::getenv("ARMSX2_FASTMEM_DISABLE_READ_64")   ? 1 : 0;
        s_dr128 = std::getenv("ARMSX2_FASTMEM_DISABLE_READ_128")  ? 1 : 0;
    ...
    static bool disable_write_32_xmm() {
        ensure_init();
        if (s_dw32_xmm < 0) s_dw32_xmm = std::getenv("ARMSX2_FASTMEM_DISABLE_WRITE_32_XMM") ? 1 : 0;
        return s_dw32_xmm != 0;
    }
    static bool disable_write_32_gpr() {
        ensure_init();
        if (s_dw32_gpr < 0) s_dw32_gpr = std::getenv("ARMSX2_FASTMEM_DISABLE_WRITE_32_GPR") ? 1 : 0;
        return s_dw32_gpr != 0;
    }
    ...
    // ARMSX2_DISABLE_SW_FASTMEM_FIX=1 で fix OFF (debug/regression 用)。 default ON (= fix 適用)。
    static int s_disable_sw_fix = -1;
    static bool apply_sw_fastmem_fix(u32 sz, bool xmm) {
        if (s_disable_sw_fix < 0) {
            s_disable_sw_fix = std::getenv("ARMSX2_DISABLE_SW_FASTMEM_FIX") ? 1 : 0;
...
//		xMOV(arg1regd, xRegister32(addr_reg));
        // [ARMSX2] @@PREPAREGS_PHYS_FIX@@: addr_reg is a physical ARM64 register number
...
static bool IsFixBios414xNoFastmemEnabled()
{
    ...
    if (s_enabled < 0)
    {
        ...
        Console.WriteLn("@@CFG@@ ARMSX2_FIX_BIOS_414X_NO_FASTMEM=%d (dual_map=%d offset=%td, V15 default ON)",
...
static void DynGen_HandlerTest(const GenDirectFn& gen_direct, int mode, int bits, bool sign = false)
{
	...
	// [ARMSX2][iter165] @@HANDLER_TEST@@: log dispatcher ptr to diagnose SIGILL BL target mismatch
...
int vtlb_DynGenReadNonQuad(u32 bits, bool sign, bool xmm, int addr_reg, vtlb_ReadRegAllocCallback dest_reg_alloc)
{
    ...
    {
        s_ee_t0_force_cfg_dyn = ARMSX2_GetRuntimeEnvBool("ARMSX2_EE_T0_FORCE_MEMREAD32", true) ? 1 : 0;
    }
...
//	const xAddressReg x86addr(addr_reg);
    // [ARMSX2] @@ADDR_REG_PHYS_FIX@@: addr_reg is always a physical ARM64 register number
...
int vtlb_DynGenReadNonQuad_Const(u32 bits, bool sign, bool xmm, u32 addr_const, vtlb_ReadRegAllocCallback dest_reg_alloc)
{
    ...
    {
        s_ee_t0_force_cfg = ARMSX2_GetRuntimeEnvBool("ARMSX2_EE_T0_FORCE_MEMREAD32", false) ? 1 : 0;
        Console.WriteLn("@@CFG@@ ARMSX2_EE_T0_FORCE_MEMREAD32=%d", s_ee_t0_force_cfg);
    }
...
//	const xAddressReg vaddr_reg(addr_reg);
    // [ARMSX2] @@WRITE_ADDR_REG_PHYS_FIX@@ (iter174): addr_reg is a physical ARM64 register
