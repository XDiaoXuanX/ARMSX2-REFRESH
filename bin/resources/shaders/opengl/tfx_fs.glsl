...
// On GLES (HAS_CLIP_CONTROL=0), PS_ZFLOOR is skipped so input_z is still gl_FragCoord.z.
// Writing it unchanged to gl_FragDepth disables depth compression and causes Z-fighting.
// Only write gl_FragDepth when we actually modified input_z (floor applied or clamp applied).
#if PS_ZWRITE && (HAS_CLIP_CONTROL || PS_ZCLAMP)
	#if NEEDS_DEPTH && PS_NO_COLOR1 && (DEPTH_FEEDBACK_SUPPORT == 2)
